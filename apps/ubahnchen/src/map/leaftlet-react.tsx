...
export const MapCity = (props: Props) => {
  const { includeChildStops, stopById, stopPairs, shapes } = props
  const onlyParents = !includeChildStops && !shapes
  ...
  for (const sp of stopPairs) {
    ...
    usedStops.add((onlyParents && s1.parent_station) || s1.stop_id)
    usedStops.add((onlyParents && s2.parent_station) || s2.stop_id)
  }

  const getStop = (stopId: string) => {
    const stop = stopById[stopId]
    if (stop.parent_station && onlyParents) return stopById[stop.parent_station]
    return stop
  }
