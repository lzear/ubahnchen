...
import pos_ from '../berlin/su/stops-positions.json'

const pos = pos_.map((p) => ({ ...p, point: [p.point[0], p.point[1] + -275] }))
...
const insertCircles = () => {
  ...
  for (const p of pos) {
    ...
    c.setAttributeNS(null, 'cx', String(p.point[0]))
    c.setAttributeNS(null, 'cy', String(p.point[1]))
...
const assignStopIds = () => {
  ...
  const stations = [
    ...stationsGroup.querySelectorAll('g > g'),
  ...
  for (const p of unused) {
    const c = document.createElementNS(svgns, 'circle')
    c.setAttributeNS(null, 'cx', p.point[0].toString())
    c.setAttributeNS(null, 'cy', p.point[1].toString())
    c.setAttributeNS(null, 'r', '3')
    c.setAttributeNS(null, 'stroke-width', '3')
    c.setAttributeNS(
      null,
      'style',
      'fill: none; stroke: red; stroke-width: 8px;',
    )
    c.dataset.stopid = p.stop_id
    // c.dataset.stopname = p.stop_name
    // c.id = p.stop_name
    svg.append(c)
  }
}
